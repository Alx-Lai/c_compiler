...
static void output_ast(VariableVector *cur_scope,
                       VariableVector *accumulative_scope, AST *ast,
                       IntVector *Context) {
  ...
  if (ast->ast_type == AST_function) {
    ...
    for (int i = 0; i < ast->body->size; i++)
      output_ast(new_scope, accumulative_scope, ast->body->arr[i], new_Context);
    ...
    output_ast(cur_scope, accumulative_scope, ast->return_value, Context);
    /* epilogue */
    outf(
        "mov %%rbp, %%rsp\n"
        "pop %%rbp\n"
    ...
    output_ast(cur_scope, accumulative_scope, ast->exp, Context);
    if (ast->type == '-') {
      outf("neg %%rax\n");
    } else if (ast->type == '!') {
      outf("not %%rax\n");
    ...
    switch (ast->type) {
      case '+':
        output_ast(cur_scope, accumulative_scope, ast->left, Context);
        outf("push %%rax\n");
        output_ast(cur_scope, accumulative_scope, ast->right, Context);
        outf(
            "pop %%rcx\n"
            "add %%rcx, %%rax\n");
        break;
    ...
    if (ast->decl_init) {
      output_ast(cur_scope, accumulative_scope, ast->decl_init, Context);
    } else {
    ...
    output_ast(cur_scope, accumulative_scope, ast->assign_ast, Context);
    ...
    counter = get_label_counter();
    output_ast(cur_scope, accumulative_scope, ast->condition, Context);
    outf(
        "cmp $0, %%rax\n"
        "je _else_%d\n",
        counter);
    output_ast(cur_scope, accumulative_scope, ast->if_body, Context);
    outf(
    ...
    /* no else -> no output */
    if (ast->else_body)
      output_ast(cur_scope, accumulative_scope, ast->else_body, Context);
    ...
    outf(
        ...
        "_continue_label_%d:\n",
        counter, counter);
    output_ast(cur_scope, accumulative_scope, ast->while_control, Context);
    outf(
        "cmp $0, %%rax\n"
        "je _end_%d\n",
        counter);
    push_back_int(Context, counter);
    output_ast(cur_scope, accumulative_scope, ast->while_body, Context);
    outf(
        "jmp _while_start_%d\n"
        "_end_%d:\n",
        counter, counter);
    ...
    push_back_int(Context, counter);
    output_ast(cur_scope, accumulative_scope, ast->do_while_body, Context);
    outf("_continue_label_%d:\n", counter);
    output_ast(cur_scope, accumulative_scope, ast->do_while_control, Context);
    outf(
        "cmp $0, %%rax\n"
    ...
    if (ast->for_init)
      output_ast(for_scope, accumulative_scope, ast->for_init, Context);
    outf("_for_cond_%d:\n", counter);
    // if empty then keep the loop
    if (ast->for_control)
      output_ast(for_scope, accumulative_scope, ast->for_control, Context);
    else
      outf("mov $1, %%rax\n");
    outf(
        "cmp $0, %%rax\n"
        ...
        counter);
    push_back_int(Context, counter);
    output_ast(for_scope, accumulative_scope, ast->for_body, Context);
    outf("_continue_label_%d:\n", counter);
    if (ast->for_post)
      output_ast(for_scope, accumulative_scope, ast->for_post, Context);
    outf(
        "jmp _for_cond_%d\n"
        "_end_%d:\n",
        counter, counter);
...
void codegen(AST *ast) {
  ...
  output_ast(NULL, global_scope, ast, NULL);
  fclose(output_f);
}